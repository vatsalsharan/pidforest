...
class PointSet:
    def __init__(self, node, points, indices):
        self.node = node
        self.points = points
        self.indices = indices
        self.dim, self.num_pts = np.shape(self.points)
        self.val, self.count = self.get_distinct()
    ...
    def get_gaps(self, axis, start, end):
        vals = self.val[axis]
        if len(vals) <= 1:
            gap = [0]
        else:
            gap = np.zeros(len(vals))
            gap[0] = (vals[0] + vals[1]) / 2 - start[axis]
            gap[-1] = end[axis] - (vals[-1] + vals[-2]) / 2
...
class Node:
    ...
    def __init__(self, depth, forest, **kwargs):
        ...
        if self.depth == 0:
            ...
            points = self.forest.points[:, indices]
            self.point_set = PointSet(self, points, indices)
        else:
            self.id_string = kwargs['id']
            self.cube = Cube(self, kwargs['start'], kwargs['end'])
            pts, indices = self.cube.filter_pts(kwargs['points'], kwargs['indices'])
            self.point_set = PointSet(self, pts, indices)
        self.density = -1
        self.child = []
        if (self.depth < self.forest.max_depth) and (self.point_set.num_pts > 1):
            self.find_split()
    ...
    def find_split(self):
        ...
        var_red = {}
        for axis in s_axes:
            gap = self.point_set.get_gaps(axis, self.cube.start, self.cube.end)
            hist = Histogram(gap / self.point_set.count[axis], self.point_set.count[axis], self.forest.max_buckets,
                             self.forest.epsilon)
        ...
        for i in range(len(self.cube.split_vals) + 1):
            ...
            kwargs.update({'points': self.point_set.points, 'indices': self.point_set.indices, 'id': new_id})
